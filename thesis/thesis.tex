...
Congested Clique, described in \cite{powerCongestClique}, is a model for distributed computing. 
It features $n$ computing nodes in a clique network, sending messages in a synchronized manner. A clique network is a complete network where each computing node is connected to all other nodes. 
In the network, each node in the network corresponds to a node in an input graph $G$. 

The computation efficiency of the congested clique model is measured by the number of (communication) rounds it takes before the algorithm converges. Each round, each node do local computation and sends $O(\log{n})$ bits to each of its neighbor nodes. 
With Congested Clique, we emphasis communication over local computation in a round. Because it is believed that the communication is the bottleneck in distributed computing. We consider local computation is free, as there are unlimited computing power and memory space on each node.

Supercomputer are high-performance computing machines that are used for large-scale scientific and engineering problems. Unlike traditional computers, supercomputers use more than one processing unit \cite{IBMsupercomputing}. The distributed natural of supercomputers makes them a good fit for Congested Clique algorithms. Therefore, we want to study how Congested Clique algorithms perform on supercomputers. 
...
We simulates Congested Clique with the above schemes to run \mstalgo. When implementing \mstalgo, the coding follows best practices and common sense. We optimize how the algorithm is implemented but do not change the algorithm itself.
\subsection{Objective}
The purpose of this work is to identify the connections between the Congested Clique model and modern distributed computing hardware, software and tools. We discuss how to simulate the Congested Clique model on supercomputers. Then we evaluate the performance, scalability of \mstalgo and overhead of simulation to the software system to show the connections.

We evaluate how the hardware is limiting the performance of the algorithm. Memory, CPU, GPU, network. And what assumptions Congested Clique makes bring the performance bottleneck. And with what setting will the bottleneck appear. We also discuss with what setting the algorithm performs as Congested Clique model expects.
...
Each node in a Congested Clique network has a unique identifier of $O(\log{n})$ bits. Each node in the network corresponds to a vertex in the input complete graph $G$. In each round of a Congested Clique of $n$ nodes, each node does local computation and sends $O(\log{n})$ bits to each of its neighbor nodes. The rounds are synchronized, meaning that all nodes start and finish the round at the same time. \cite{powerCongestClique}
...
\subsection{\mstalgo}
\mstalgo is an algorithm developed by Zvi Lotker, Boaz Patt-Shamir, Elan Pavlov, and David Peleg in 2005. It solves the Minimum Spanning Tree ($MST$) problem in $O(\log{\log{n}})$ communication rounds in the Congested Clique model. There are faster and new algorithms for the $MST$ problem in the Congested Clique model e.g. \cite{MSTLogStar} and \cite{MSTConstant}, but \mstalgo is chosen for the following reasons: 
\begin{itemize}
	\item[--] \mstalgo is a Congested Clique algorithm. The properties of \mstalgo generalize to the whole algorithm category (will be shown later).
...
The same simplification is applied to the CUDA implementation.

\subsection{Profiling}
%  mention memory copy device to host and host to device, kernel function time, 
Profiling is done with nvprof. TODO what nvprof.Profiling result shows the time spent on the memory copying and the kernel functions. The profiling result shows the time spent on each kernel function. The time spent on each kernel function is the time spent on the communication and local computation in each round.
\clearpage
...
% capability of the GPU (How many edges)
...
\begin{thebibliography}{99}
	...
	\bibitem{IBMsupercomputing} "Supercomputing," IBM, Accessed: Jul. 16, 2024. [Online]. Available: \url{https://www.ibm.com/topics/supercomputing}

	\bibitem{MSTConstant} K. Nowicki, "A deterministic algorithm for the MST problem in constant rounds of congested clique," \textit{arXiv.org}, Dec. 9, 2019. [Online]. Available: \url{https://arxiv.org/abs/1912.04239}

	\bibitem{MSTLogStar} M. Ghaffari and M. Parter, "MST in Log-Star rounds of congested clique," \textit{ACM}, 2016. [Online]. Available: \url{https://doi.org/10.1145/2933057.2933103}
\end{thebibliography}
